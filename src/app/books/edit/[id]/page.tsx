...
export default function Edit({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        getBookById(params.id).then((data: any) => {
            console.log(data)
            setValue("bookTitle", data.book.bookTitle);
            setValue("language", data.book.language);
            setValue("publishedYear", data.book.publishedYear);
            setValue("category", data.book.category);
            setValue("description", data.book.description);
        })
    })
    ...
    return (
        <>
            <main className="panel px-5 py-5">
                <h1 className="text-lg font-semibold mb-2">Edit Book</h1>
                <hr className='mb-6' />
